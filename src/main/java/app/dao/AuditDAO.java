...
public class AuditDAO {
    ...
    // 1. Insertar Log: Captura acciones con o sin ID de usuario
    ...
    // 2. Obtener historial para el Admin o el Usuario (Seguridad de cuenta)
    ...
    // 3. Obtener auditoría global de todo el sistema
    public List<AuditLog> getGlobalLogs() {
        List<AuditLog> list = new ArrayList<>();
        // Traemos los últimos 200 eventos globales para el panel administrativo
    ...
    // 4. Sistema Anti-Fuerza Bruta: Cuenta fallos en un periodo de tiempo
    public int countRecentFailures(String identifier, String action) {
        String sql = "SELECT COUNT(*) FROM AuditLogs WHERE UserIdentifier = ? AND Action = ? AND CreatedAt > DATE_SUB(NOW(), INTERVAL 30 MINUTE)";
        try (Connection conn = ConnectionDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, identifier);
    ...
    // 5. Mapeador sincronizado con LocalDateTime
